...
class SyncTests(unittest.TestCase):
    ...
    def test_manual_subscription_write_works_for_anthropic(self):
        ok, _ = account_switcher.write_subscription(
            "settings.json", "2026-03-30T12:01", 30, None, None)

        self.assertTrue(ok)
        self.assertEqual(self.subs_data()["settings.json"]["days"], 30)
    ...
    def test_account_config_hides_subscription_for_openai_only(self):
        ...
        self.assertIsNone(config["subscriptionFields"])

        ok, _, config = account_switcher.read_account_config("settings_glm.json")
        self.assertTrue(ok)
        self.assertEqual(config["subscription"], {})  # пустая ручная форма
        self.assertEqual(config["subscriptionFields"], {
            "email": True, "paidAt": False, "days": False,
        })

        ok, _, config = account_switcher.read_account_config("settings.json")
        self.assertTrue(ok)
        self.assertEqual(config["subscription"], {})  # Anthropic тоже доступен
        self.assertEqual(config["subscriptionFields"], {
            "email": False, "paidAt": True, "days": True,
        })
