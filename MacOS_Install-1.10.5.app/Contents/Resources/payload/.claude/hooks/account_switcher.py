...
Активные пользовательские настройки — `~/.claude/settings.json`. Аккаунты
...
копией выбранного с общими настройками из `settings-hooks.json` в начале.
Проектные настройки Claude Code читает отдельным слоем.
...
from settings_hooks import read_settings, with_shared_settings
...
# Capability order for the supported Codex models. model/list order and
# isDefault describe the picker default, not the most capable model.
...
def openai_role_models(models) -> dict[str, str]:
    """Use three distinct, available models; never guess unknown model IDs."""
    if not isinstance(models, list):
        return {}
    available = {m.get("id") for m in models
                 if isinstance(m, dict) and isinstance(m.get("id"), str)
                 and not m.get("hidden")}
    ranked = [model for model in OPENAI_MODEL_PRIORITY if model in available]
    if len(ranked) < len(OPENAI_ROLE_KEYS):
        return {}
    return dict(zip(OPENAI_ROLE_KEYS, ranked))


def sync_openai_models(models=None) -> list[str]:
    """Repair role aliases in the saved profile and its active bridge copy.

    Keep the selected model, context settings, hooks and credentials intact.
    An unavailable/incomplete catalog leaves the last working mapping alone.
    """
    if models is None:
        snapshot = codex_bridge_manager.account_snapshot()
        models = snapshot.get("models") if isinstance(snapshot, dict) else None
    roles = openai_role_models(models)
    if not roles:
        return []
    changed = []
    bridge_url = (f"http://{codex_bridge_manager.BRIDGE_HOST}:"
                  f"{codex_bridge_manager.BRIDGE_PORT}")
    for path in (os.path.join(CLAUDE_DIR, "settings_openai.json"), SETTINGS_FILE):
        try:
        ...
        if not isinstance(env, dict) or str(env.get("ANTHROPIC_BASE_URL", "")).rstrip("/") != bridge_url:
            continue
        if all(env.get(key) == value for key, value in roles.items()):
            continue
        env.update(roles)
...
_HOOK_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
ACCOUNT_EVENTS_LOG = os.path.join(
    _HOOK_ROOT, "hooks-runtime", "account-events.jsonl"
)
...
EXCLUDED = {"settings.local.json", "settings-hooks.json"}
...
ZAI_CACHE_FILE = os.path.join(
    _HOOK_ROOT, "hooks-runtime", "zai-subs-cache.json"
)
...
ZAI_USAGE_CACHE_FILE = os.path.join(
    _HOOK_ROOT, "hooks-runtime", "zai-usage-cache.json"
)
ZAI_USAGE_TTL_SEC = 300      # квота тратится на глазах: чаще подписки
USAGE_CONFIG_FILE = os.path.join(_HOOK_ROOT, "patches", "claude-custom-config.toml")
...
def _openai_usage(rate_limits: Any) -> dict | None:
    ...
    for key, label, title in (
        ...
        windows.append({
            ...
            "percent": percent, "resetAt": resets_at, "resetsInSec": max(0, left) if left is not None else None,
            "expired": left is not None and left <= 0,
        })
    if not windows:
        return None
    return {"windows": windows, "ageSec": 0, "observedAt": observed,
...
def openai_account(snapshot=None) -> dict:
    if snapshot is None:
        snapshot = codex_bridge_manager.account_snapshot()
    if not snapshot:
...
def anthropic_usage() -> dict | None:
    ...
    return {"windows": windows, "ageSec": age, "observedAt": fetched / 1000 if age is not None else None}
...
def list_accounts(*, refresh_usage: bool = False, usage_recorder=None) -> list[dict]:
    ...
    for path in sorted(glob.glob(os.path.join(CLAUDE_DIR, "settings*.json"))):
        ...
        if info["oauth"]:
            ...
            if openai_identity is None:
                openai_snapshot = codex_bridge_manager.account_snapshot() or {}
                openai_identity = openai_account(openai_snapshot)
            sources["openai_snapshot"] = openai_snapshot
            ...
            usage = zai_usage(env, force_refresh=refresh_usage)
            sources["zai_sample"] = usage or {}
            if usage:
                entry["usage"] = usage
                entry["usageZai"] = True
...
def read_account_config(filename: str) -> tuple[bool, str, dict]:
    ...
    ручная часть данных подписки, а `subscriptionFields` сообщает
    webview, какие именно поля показывать. У Anthropic email приходит
    из credentials, у Z.AI период — из его API; повторно вводить их
    руками не предлагаем. Для OpenAI раздел — None: вся подписка
    пишется автоматически из Codex id_token (codex_id_token_sync.py),
    ручного раздела быть не должно.
    """
    ok, message, data = _load_account(filename)
    if not ok:
        return False, message, {}
    env = data.get("env")
    description = _describe(source_path(filename))
    subscription = None
    subscription_fields = None
    if description["provider"] != "openai":
        # Пустой словарь здесь значим: ручной записи ещё нет, но webview
        # должен показать пустые поля и дать её создать. None оставляем
        # только OpenAI, где срок синхронизируется из Codex id_token и
        # ручной раздел намеренно скрыт.
        subscription = read_subscription(filename) or {}
        is_zai = _zai_host(env if isinstance(env, dict) else {}) is not None
        subscription_fields = {
            "email": not description["oauth"],
            "paidAt": not is_zai,
            "days": not is_zai,
        }
    return True, "", {
...
def write_account_config(filename: str, env, settings) -> tuple[bool, str]:
    ...
    path = source_path(filename)
    try:
        # Validate the shared layer before modifying either copy.
        data = with_shared_settings(CLAUDE_DIR, data)
        _write_json_atomic(path, data)
...
def _write_json_atomic(path: str, data: dict) -> None:
    ...
    """
    if (os.path.abspath(os.path.dirname(path)) == os.path.abspath(CLAUDE_DIR)
            and (ACCOUNT_NAME_RE.fullmatch(os.path.basename(path))
                 or os.path.abspath(path) == os.path.abspath(BACKUP_FILE))):
        data = with_shared_settings(CLAUDE_DIR, data)
...
def zai_usage_cache_ttl() -> float:
    try:
        with open(USAGE_CONFIG_FILE, "rb") as handle:
            value = tomllib.load(handle).get("zaiUsageCacheTtlSec", ZAI_USAGE_TTL_SEC)
        if isinstance(value, (int, float)) and not isinstance(value, bool) and math.isfinite(value) and value >= 0:
            return value
    except (OSError, ValueError, TypeError, AttributeError):
        pass
    return ZAI_USAGE_TTL_SEC


def zai_usage(env: dict, *, force_refresh: bool = False) -> dict | None:
    ...
    cache_ttl = zai_usage_cache_ttl()
    payload = _zai_cache_payload(
        token, ZAI_USAGE_CACHE_FILE, 1,
        float("inf") if force_refresh else cache_ttl, ZAI_FAIL_TTL_SEC) if cache_ttl else None
    ...
    def failed_refresh():
        if force_refresh and payload and isinstance(payload.get("result"), dict):
            return {**payload["result"], "ageSec": _zai_cache_age(payload), "refreshFailed": True,
                    "sourceLabel": "Данные Z.AI · не удалось обновить"}
        if cache_ttl:
    ...
    if isinstance(items, list):
        for item in items:
            ...
            if isinstance(reset_ms, bool) or not isinstance(reset_ms, (int, float)):
                ...
                left = int(reset_ms / 1000 - observed_at)
            expired = left is not None and left <= 0
            windows.append({
                "key": key,
                "label": label,
                ...
                "percent": 0 if expired else percent,
                "resetsInSec": None if expired else left,
                "resetAt": reset_ms / 1000 if left is not None else None,
                "expired": expired,
            })
    if not windows:
        return failed_refresh()
    result = {"windows": windows, "sourceLabel": "Данные Z.AI", "observedAt": observed_at}
    if cache_ttl:
...
def switch_account(target: str, revert: bool = False) -> tuple[bool, str]:
    ...
    if target != BASE_NAME and not os.path.isfile(target_path):
    ...
    if target == "settings_openai.json":
        try:
            ...
            return False, f"Ошибка обновления моделей OpenAI: {exc}"

    current = get_current_account()
    if current == target:
        try:
            data = read_settings(SETTINGS_FILE) if os.path.isfile(SETTINGS_FILE) else {}
            _write_json_atomic(SETTINGS_FILE, data)
        except (OSError, ValueError) as exc:
            return False, f"Ошибка обновления настроек: {exc}"
    ...
    try:
        if target == BASE_NAME:
            ...
            _write_json_atomic(SETTINGS_FILE, read_settings(BACKUP_FILE))
            os.remove(BACKUP_FILE)
            if os.path.isfile(ACTIVE_MARKER):
                os.remove(ACTIVE_MARKER)
        else:
            # Parse and merge before creating a backup or changing the marker.
            data = with_shared_settings(CLAUDE_DIR, read_settings(target_path))
            # Уход с оригинала: сохраняем его один раз. Повторное
            # копирование затёрло бы бэкап настройками чужого провайдера.
            if not os.path.isfile(BACKUP_FILE):
                if not os.path.isfile(SETTINGS_FILE):
                    _write_json_atomic(SETTINGS_FILE, {})
