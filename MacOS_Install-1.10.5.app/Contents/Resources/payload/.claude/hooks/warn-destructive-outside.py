...
import shlex
import sys
from hook_paths import RUNTIME
...
def is_inside(path_str: str, safe_roots: list[str]) -> bool:
    # Compare canonical path components using the host OS rules. A literal
    # '/' prefix fails for Windows backslashes, drive casing and junctions.
    target = os.path.normcase(os.path.realpath(path_str))
    for root in safe_roots:
        ...
        try:
            ...
            # Different drives (or incompatible path forms) are outside.
            continue
    return False
