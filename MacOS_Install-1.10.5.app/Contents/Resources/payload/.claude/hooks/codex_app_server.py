...
def _extension_binary_candidates() -> list[str]:
    ...
    if os.name == "nt":
        arch_names = ("windows-aarch64",) if machine in ("arm64", "aarch64") else ("windows-x86_64",)
    elif sys.platform == "darwin":
        arch_names = {
            "x86_64": ("macos-x86_64", "darwin-x86_64", "darwin-x64"),
            "amd64": ("macos-x86_64", "darwin-x86_64", "darwin-x64"),
            "arm64": ("macos-aarch64", "macos-arm64", "darwin-aarch64", "darwin-arm64"),
            "aarch64": ("macos-aarch64", "macos-arm64", "darwin-aarch64", "darwin-arm64"),
        }.get(machine, ())
    binary_name = "codex.exe" if os.name == "nt" else "codex"
    patterns = []
    for arch in arch_names:
        patterns.append(os.path.expanduser(
            f"~/.vscode/extensions/openai.chatgpt-*/bin/{arch}/{binary_name}"
        ))
        patterns.append(os.path.expanduser(
            f"~/.vscode-server/extensions/openai.chatgpt-*/bin/{arch}/{binary_name}"
        ))
    found: list[str] = []
