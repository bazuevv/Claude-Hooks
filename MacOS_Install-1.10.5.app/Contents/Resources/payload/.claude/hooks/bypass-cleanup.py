...
def main() -> int:
    ...
    runtime_dir = bypass_dir(project_dir)

    if isinstance(current_session, str) and re.fullmatch(r"[A-Za-z0-9_-]+", current_session):
        try:
    ...
    for marker in runtime_dir.iterdir():
        if not marker.is_file() or not UUID_RE.fullmatch(marker.name):
            continue
