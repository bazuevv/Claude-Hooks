...
from host_platform import vscode_data_dirs

HOME = os.path.expanduser("~")
EXT_GLOB = os.path.join(HOME, ".vscode/extensions/anthropic.claude-code-*")

from hook_paths import ROOT as HOOK_ROOT, RUNTIME, GLOBAL_INSTALL

PROJECT_DIR = os.environ.get("CLAUDE_PROJECT_DIR")
if not PROJECT_DIR:
    PROJECT_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))

PATCHES_DIR = os.path.join(str(HOOK_ROOT), "patches")
...
REQUIRED_PARAMS = [
    ("userMessageOutsideCollapse", lambda v: isinstance(v, bool), "должен быть true или false"),
    ("userMessageInitialHeightPx", lambda v: isinstance(v, int) and not isinstance(v, bool) and v >= 60,
     "должен быть целым числом не меньше 60 px"),
    ("userMessageMediumHeightPx", lambda v: isinstance(v, int) and not isinstance(v, bool) and v > 60,
     "должен быть целым числом больше начальной высоты"),
    ("userMessageMaxHeightPx", lambda v: isinstance(v, int) and not isinstance(v, bool) and v > 60,
     "должен быть целым числом больше средней высоты"),
    ("markdownAnimateSentences", lambda v: isinstance(v, bool), "должен быть true или false"),
    ("markdownWholeSentences", lambda v: isinstance(v, bool), "должен быть true или false"),
    ("markdownWholeWords", lambda v: isinstance(v, bool), "должен быть true или false"),
    ("markdownShowPartialParagraph", lambda v: isinstance(v, bool), "должен быть true или false"),
    ("chatAutoScroll", lambda v: isinstance(v, bool), "должен быть true или false"),
    ("cpuProbeEnabled", lambda v: isinstance(v, bool), "должен быть true или false"),
    ("cpuProbeIntervalSec", lambda v: isinstance(v, int) and not isinstance(v, bool) and 1 <= v <= 60,
     "должен быть целым числом от 1 до 60 секунд"),
    ("cpuProbeMaxMinutes", lambda v: isinstance(v, int) and not isinstance(v, bool) and 1 <= v <= 1440,
     "должен быть целым числом от 1 до 1440 минут"),
    ("markdownRenderIntervalMs",
     lambda v: isinstance(v, int) and not isinstance(v, bool) and v >= 0,
     "должен быть неотрицательным целым числом (мс)"),
    ("lazyCollapsedThinking", lambda v: isinstance(v, bool), "должен быть true или false"),
    ("perfNativeMarkdown", lambda v: isinstance(v, bool), "должен быть true или false"),
    ("messageCostsEnabled", lambda v: isinstance(v, bool), "должен быть true или false"),
    ...
    ("zaiUsageCacheTtlSec",
     lambda v: isinstance(v, int) and not isinstance(v, bool) and v >= 0,
     "должен быть неотрицательным целым числом (сек)"),
    ("accountUsageBackgroundEnabled", lambda v: isinstance(v, bool), "должен быть true или false"),
    ("accountUsageBackgroundIntervalSec",
     lambda v: isinstance(v, int) and not isinstance(v, bool) and 60 <= v <= 86400,
     "должен быть целым числом от 60 до 86400 секунд"),
    ("messageCostAccounts",
     lambda v: isinstance(v, dict) and all(isinstance(a, dict)
         and a.get("billing", "api") in ("api", "subscription")
         and isinstance(a.get("history_id", ""), str) for a in v.values()),
     "должен содержать параметры аккаунтов: billing (api/subscription) и необязательный history_id"),
    (
        "logs",
    ...
    (
        "userMessageTimestampFormat",
        lambda v: isinstance(v, str) and bool(v.strip()),
        "должен быть непустой строкой формата даты и времени",
    ),
    (
        "userMessageCompletionTimeFormat",
        lambda v: isinstance(v, str) and bool(v.strip()),
        "должен быть непустой строкой формата времени завершения",
    ),
    (
        "bypassButton",
        lambda v: isinstance(v, bool),
    ...
    *[(key, lambda v: isinstance(v, int) and not isinstance(v, bool) and v > 0,
       "должен быть положительным целым числом (мин)")
      for key in ("cacheKeepaliveTtlMinutesAnthropic", "cacheKeepaliveTtlMinutesOpenai",
                  "cacheKeepaliveTtlMinutesZai", "cacheKeepaliveTtlMinutesCustom")],
    (
        "serverLog",
        lambda v: isinstance(v, bool),
        "должен быть true или false",
...
USER_SETTINGS_PATHS = [
    ...
    for directory in vscode_data_dirs()
...
def _make_symlink(target: str, link_path: str) -> bool:
    ...
    if os.name == "nt":
        # Windows symlinks require Developer Mode or elevated privileges.
        content = _read(target_abs)
        if os.path.isfile(link_path) and _read(link_path) == content:
            return False
        ext_patch.atomic_write(link_path, content)
        return True
    try:
...
def _read_config() -> tuple[dict, list[str]]:
    ...
    initial = cfg.get("userMessageInitialHeightPx")
    medium = cfg.get("userMessageMediumHeightPx")
    maximum = cfg.get("userMessageMaxHeightPx")
    if (all(isinstance(v, int) and not isinstance(v, bool) for v in (initial, medium, maximum))
            and not initial < medium < maximum):
        issues.append("высоты сообщений должны строго возрастать: initial < medium < max")
...
def _pid_file_path() -> str:
    return os.path.join(str(RUNTIME), "http-server.pid")
...
def _spawn_lock_path() -> str:
    return os.path.join(str(RUNTIME), "http-server.lock")


def _acquire_spawn_lock():
    """Nonblocking process lock on macOS/Linux and Windows alike."""
    fh = None
    try:
        os.makedirs(os.path.dirname(_spawn_lock_path()), exist_ok=True)
        fh = open(_spawn_lock_path(), "a+b")
        if os.name == "nt":
            import msvcrt
...
def _ensure_http_server_locked() -> list[str]:
    ...
    if status is not None:
        ...
        if running >= sources - 0.5 and status.get("hook_root") == str(HOOK_ROOT):
            return []
        _hlog(
