...
CSP_CONNECT_SRC = "connect-src https://api.anthropic.com http://localhost:* http://127.0.0.1:18923"
...
try {
  (function () {
    ...
    function runtimeDirs() {
      var out = [path.join(require("os").homedir(), ".claude", "hooks-runtime")];
    ...
    function belongsToWindow(request) {
      if (request && request.windowId) return request.windowId === windowId;
      ...
      return folders.some(function (folder) {
        return canonical(folder.uri.fsPath) === canonical(request.project);
      });
    }
    ...
    // A fresh identity routes actions to this exact extension host, including
    // empty windows and new sessions that have no transcript on disk yet.
    var windowId = require("crypto").randomBytes(16).toString("hex");
    var globalRoot = path.join(require("os").homedir(), ".claude");
    var windowsDir = path.join(globalRoot, "hooks-runtime", "windows");
    var supervisorChild = null;
    var nextSupervision = 0;
    function workspaceProjects() {
      return (vscode.workspace.workspaceFolders || []).map(function (f) { return f.uri.fsPath; });
    }
    function maintainHooks() {
      var projects = workspaceProjects();
      try {
        fs.mkdirSync(windowsDir, {recursive: true});
        var record = path.join(windowsDir, windowId + ".json");
        fs.writeFileSync(record + ".tmp", JSON.stringify({windowId: windowId, pid: process.pid,
          ts: Date.now(), projects: projects}), {mode: 384});
        fs.renameSync(record + ".tmp", record);
      } catch (e) { log("window registration: " + e); }
      panels.forEach(function (panel) {
        try {
          Promise.resolve(panel.webview.postMessage({__claudeCustom: "window-context", windowId: windowId}))
            .catch(function () {});
        } catch (e) {}
      });
      if (supervisorChild || Date.now() < nextSupervision) return;
      nextSupervision = Date.now() + 30000;
      try {
        var script = path.join(globalRoot, "hooks", "hook_supervisor.py");
        if (!fs.existsSync(script)) return;
        var python = fs.readFileSync(path.join(globalRoot, "install-state", "python-path.txt"), "utf8").trim();
        if (!path.isAbsolute(python) || !fs.existsSync(python)) return;
        var args = ["-B", script];
        projects.forEach(function (project) { args.push("--project", project); });
        var environment = Object.assign({}, process.env, {PYTHONUTF8: "1", PYTHONIOENCODING: "utf-8"});
        delete environment.CLAUDE_PROJECT_DIR;
        supervisorChild = require("child_process").spawn(python, args,
          {windowsHide: true, stdio: "ignore", env: environment, cwd: globalRoot});
        var child = supervisorChild;
        var timeout = setTimeout(function () { try { child.kill(); } catch (e) {} }, 90000);
        function finished() { clearTimeout(timeout); if (supervisorChild === child) supervisorChild = null; }
        child.on("error", function (err) { log("supervisor: " + err); finished(); });
        child.on("exit", function (code) { if (code) log("supervisor exit=" + code); finished(); });
      } catch (e) { supervisorChild = null; log("supervisor: " + e); }
    }
    if (typeof vscode.workspace.onDidChangeWorkspaceFolders === "function") {
      vscode.workspace.onDidChangeWorkspaceFolders(function () { nextSupervision = 0; maintainHooks(); });
    }
    process.once("exit", function () {
      try { fs.unlinkSync(path.join(windowsDir, windowId + ".json")); } catch (e) {}
    });
    ...
    function scan() {
      ...
      for (var i = 0; i < dirs.length; i++) {
        ...
        if (!belongsToWindow(req)) continue;
        ...
        if (req.windowId && !req.project) {
          setTimeout(function () { vscode.commands.executeCommand("workbench.action.reloadWindow"); }, RESTART_DELAY_MS);
          return;
        }
    ...
    function consumeReloadRequest() {
      ...
      for (var i = 0; i < dirs.length; i++) {
        ...
        if (data.project && !belongsToWindow(data)) continue;
