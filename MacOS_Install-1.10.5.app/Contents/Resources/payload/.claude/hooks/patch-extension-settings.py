...
CACHE_GLOBS = [
    os.path.join(directory, "CachedProfilesData", "*", "extensions.user.cache")
    for directory in vscode_data_dirs()
]

from hook_paths import ROOT as HOOK_ROOT, RUNTIME
...
CANONICAL_CONFIG = os.path.join(
    str(HOOK_ROOT), "patches", "claude-custom-config.toml"
)
...
def _load_translations(locale: str) -> dict:
    ...
    if locale == "en":
        return {}
    path = os.path.join(
        str(HOOK_ROOT), "patches", "locales", f"{locale}.json"
    )
    try:
...
def _marker_path() -> str:
    return os.path.join(
        str(RUNTIME), "ext-settings-applied.json"
    )
